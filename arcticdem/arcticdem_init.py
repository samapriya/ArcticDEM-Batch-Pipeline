...
def reformat():
    ...
    for files in filelist:
        ...
        sink = fiona.open(outfile, 'w', driver='ESRI Shapefile', schema=output_schema, crs=src.crs)
        for feature in src:
        ...
        for filename in index_filenames:
            try:
                if 'Strip' in filename:
                    ...
                    urllib.request.urlretrieve("http://data.pgc.umn.edu/elev/dem/setsm/ArcticDEM/indexes/"+filename, out_file_path)
                    zip_ref = zipfile.ZipFile(out_file_path)
                    for file in zip_ref.namelist():
                        if zip_ref.getinfo(file).filename.endswith(".shp"):
                            print('Using release version for Strip: {}'.format(file.split('.')[0]))
                    for file in zip_ref.namelist():
                    ...
                    for file in zip_ref.namelist():
                        if zip_ref.getinfo(file).filename.endswith(".shp"):
                            print('Using release version for Tile: {}'.format(file.split('.')[0]))
                    for file in zip_ref.namelist():
                        zip_ref.extract(file, pth)
            except Exception as e:
                print("The URL is invalid. Please double check the URL or error. {}".format(e))
    reformat()
#search_index()
