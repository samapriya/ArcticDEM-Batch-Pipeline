...
def unpack(inpath,outfolder):
    ...
    for member in t.getmembers():
        if member.name.startswith('index'):
            try:
                fpath = outfolder
                if not os.path.exists(os.path.join(fpath,os.path.basename(member.name))):
                    print('Processing: {} and Extracting: {}'.format(os.path.basename(inpath),os.path.basename(member.name)))
                    t.extract(member, fpath)
                else:
                    print('File already exists SKIPPING: {}'.format(os.path.basename(member.name)))
